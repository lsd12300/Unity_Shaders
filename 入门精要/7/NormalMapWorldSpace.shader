...
// 世界空间 法线贴图
//      优点： 需要 Cubemap 进行环境映射的情况，必须使用 世界空间
//      缺点： 
    ...
    SubShader {
        Pass {
            ...
            fixed4 frag(v2f i) : SV_Target {
                float3 worldPos = float3(i.TtoW0.w, i.TtoW1.w, i.TtoW2.w);      // 世界空间 坐标
                fixed3 lightDir = normalize(UnityWorldSpaceLightDir(worldPos));
                fixed3 viewDir = normalize(UnityWorldSpaceViewDir(worldPos));
                
                fixed3 bump = UnpackNormal(tex2D(_BumpMap, i.uv.zw));       // 切线空间  法线
                bump.xy *= _BumpScale;
